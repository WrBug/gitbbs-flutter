    ...
    for (int i = 0; i < rowsLength; i++) {
      ...
      for (int j = start; j < end; j++) {
        if (!widget.symmetry && _tags.isNotEmpty) {
          ...
          widthTag = txtWidth + (tag.length < 4
              ? padding *
              (margin > 10 ? 3 : 2 + fontSize / (_initFontSize * 2.5))
              : padding * 2.3);
        ...
        child: Container(
            ...
            child: OutlineButton(
                ...
                borderSide: widget.borderSide ??
                    BorderSide(color: (widget.activeColor ?? Colors.blueGrey)),
                child:
                (tag.icon != null) ?
                FittedBox(
                  child: Icon(
                ...
                onPressed: () {
                  ...
                  setState(() {
                    (widget.singleItem) ? tag.active = true : tag.active =
                    !tag.active;
  ...
  TextStyle _textStyle(Tag tag) {
    if (widget.textStyle != null)
      return widget.textStyle.apply(
        color: tag.active ? (widget.textActiveColor ?? Colors.white) : (widget
